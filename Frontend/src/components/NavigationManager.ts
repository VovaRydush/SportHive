  ...
  private renderUserNav(role: string | null) {
    ...
    setTimeout(() => {
      document.getElementById('judge-btn')?.addEventListener('click', () => {
        new JudgeModal().show('1','5');
      });
      document.getElementById('team-btn')?.addEventListener('click', () => {
        var newModal = new CreateTeamModal();
      newModal.show();
      });
      document.getElementById('profile-btn')?.addEventListener('click', () => {
        new UserProfile('app',localStorage.getItem('login') || "").render();
      });
      document.getElementById('profi-btn')?.addEventListener('click', () => {
        var login = localStorage.getItem('login')
        new TrainerProfile('app',login || "").render();
      });
